...
-- describe (nom table);
--source /home/etd/BaseDeDonne/exercices/semaine4/exercices2.sql
...
SELECT
  name AS `nom`,
  due_date AS `date d'échéance`
FROM tasks
WHERE MONTH(due_date) BETWEEN 6 AND 8;
...
due_date as `date d'échéance`
 from tasks
where due_date between '2025-06-01' and '2025-08-31';
...
ifnull(time_format(actual_duration, "%Hh%i"), "") as `actual_duration`,
ifnull(completed_date,"") as `completed_date`,
  case 
...
delete
from tasks
where actual_duration is null and completed_date is not null
returning name,time_format(estimated_duration, '%k') as `estimated_duration`, completed_date, due_date;